...
update_upmem=$projectdir/TFM_UPDATE_upmem.sh
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b boot -m  RE_BL2_PERSO_ADDRESS -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot0 -m  RE_IMAGE_FLASH_ADDRESS_SECURE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot1 -m  RE_IMAGE_FLASH_ADDRESS_NON_SECURE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot2 -m  RE_IMAGE_FLASH_SECURE_UPDATE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot3 -m  RE_IMAGE_FLASH_NON_SECURE_UPDATE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot4 -m  RE_IMAGE_FLASH_ADDRESS_DATA_SECURE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot5 -m  RE_IMAGE_FLASH_ADDRESS_DATA_NON_SECURE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot6 -m  RE_IMAGE_FLASH_DATA_SECURE_UPDATE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b slot7 -m  RE_IMAGE_FLASH_DATA_NON_SECURE_UPDATE -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b loader -m  RE_IMAGE_FLASH_LOADER_ADDRESS -s 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b cfg_loader -m  RE_EXT_LOADER -s 0 -p 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b app_image_number -m  RE_APP_IMAGE_NUMBER -s 0 -p 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b s_data_image_number -m  RE_S_DATA_IMAGE_NUMBER -s 0 -p 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
...
command=$cmd" "$imgtool" flash --layout "$preprocess_bl2_file" -b ns_data_image_number -m  RE_NS_DATA_IMAGE_NUMBER -s 0 -p 0 "$update_upmem
$command  >> $projectdir"/output.txt"
ret=$?
if [ $ret != 0 ]; then
echo "postbuild.sh failed"
