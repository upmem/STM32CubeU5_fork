...
/* Set to 1 to optimize RAM usage. If set to 1 the StSafeA_Handle_t.InOutBuffer
   used through the Middleware APIs is shared with the application between each
   command & response. It means that every time the MW API returns a TLVBuffer pointer,
   it returns in fact a pointer to the shared StSafeA_Handle_t.InOutBuffer.
   As consequence the user shall copy data from given pointer into variable defined
   by himself in case data need to be stored.
   If set to 0 the user must specifically allocate (statically or dynamically)
   a right sized buffer to be passed as parameter
   to the Middleware command API */
#define STSAFEA_USE_OPTIMIZATION_SHARED_RAM             1U

/* Set to 1 to optimize RAM/Flash usage, when Host MAC and encryption features
   are not required (eg. Authentication use case) */
#define STSAFEA_USE_OPTIMIZATION_NO_HOST_MAC_ENCRYPT    0U

/* Set to 1 to have CRC16_CCIT Table already calculated and placed in Flash as const.
   Set to zero to dynamically calculate it in RAM */
...
/* Increasing polling frequency to optimize delays */
#define STSAFEA_I2C_POLLING_STEP                1U
